...
class TextTools:
    ...
    def _register_text(self):
        ...
        self.tools["read_runs_live"] = {
            "description": "Read the text at an address as the formatted runs it is made of, each with its own address, font, colour and language. Use this before rewriting text that is not uniformly formatted: replacing such a range in one go flattens it, so a monospace term or a coloured phrase inside it loses its look. What it reads follows the address. A block ({\"paragraph\": N, \"through\": M}) reads every paragraph in it, each run saying which paragraph it is in, with `paragraphs` and `paragraphs_read` beside them. An **anchor** — the `address` get_cursor_info, find_text or read_paragraphs handed back — reads whatever that anchor covers: the anchor on a **selection** of nine paragraphs reads all nine, each held by an anchor of its own that its runs are addressed by, with `paragraphs_read` saying how many and `paragraph` coming back null, since no paragraph number is worked out (a paragraph has none in UNO; counting one walks the body). An anchor over one paragraph, or over part of one such as a search hit, reads just that. Either way the cap is 50 paragraphs, after which the answer says `truncated`",
            "parameters": {
                "type": "object",
                "properties": {
                    "address": {
