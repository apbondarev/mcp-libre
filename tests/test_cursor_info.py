...
def test_reports_paragraph_index_and_document_offset(bridge):
    ...
    # Both are counted only when asked for: a paragraph has no number in UNO,
    # so finding one walks the body, and the character offset pulls every
    # paragraph's text over the bridge on the way.
    result = bridge.get_cursor_info(number=True, character_offset=True,
                                    doc=doc)
...
def test_fails_for_non_writer_document(bridge):
    result = bridge.get_cursor_info(doc=FakeCalcDoc())
...
def test_locate_range_reports_the_address_of_a_range(bridge):
    ...
    address, paragraph_cursor, chars_before = bridge._locate_range(
        doc, target, count_characters=True)
...
def test_tool_is_registered_and_dispatches_to_the_bridge():
    ...
    # means the active document. The two counting flags are off by default,
    # because the number a paragraph does not have costs a walk of the body.
    assert set(server.tools["get_cursor_info_live"]["parameters"]
               ["properties"]) == {"document", "number", "character_offset"}
...
# --- where the caret is, without counting the paragraphs before it ---------
#
# Measured on a real 6981-paragraph guide: the walk that gives a paragraph its
# number costs 0.55 ms a paragraph, so this call took three seconds with the
# caret at paragraph 4072 and nearly four at 6900 — to answer a question the
# caller asks in order to act there, which an anchor answers for two calls.

def test_the_caret_is_reported_without_walking_the_body(bridge, monkeypatch):
    doc = writer_doc(PARAGRAPHS, caret=(2, 4))
    ...
    assert result["success"] is True
    assert result["cursor"]["offset_in_paragraph"] == 4
    assert result["cursor"]["paragraph_index"] is None
    assert result["cursor"]["document_offset"] is None
    assert "number: true" in result["note"]


def test_the_caret_comes_back_as_an_anchor_that_names_its_paragraph(bridge):
    doc = writer_doc(PARAGRAPHS, caret=(2, 4))
...
def test_the_number_comes_with_the_anchor_when_it_is_asked_for(bridge):
    doc = writer_doc(PARAGRAPHS, caret=(2, 4))
